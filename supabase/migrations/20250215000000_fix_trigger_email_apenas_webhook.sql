-- ==========================================
-- CORREÇÃO: Trigger só deve enviar email quando pagamento foi confirmado via webhook
-- MAS SEMPRE deve gerar letras quando pedido é marcado como paid
-- ==========================================
-- PROBLEMA: O trigger estava enviando email para QUALQUER pedido marcado como paid,
-- mesmo quando marcado manualmente ou por scripts de correção.
-- 
-- SOLUÇÃO: 
-- - O trigger só deve enviar email se o pedido tiver indicadores válidos
--   de pagamento confirmado (cakto_payment_status = 'approved' ou stripe_payment_intent_id)
-- - O trigger SEMPRE deve tentar gerar letras se não foram geradas ainda (fallback)
-- ==========================================

-- Instalar pg_net se não estiver instalado (necessário para chamar Edge Functions)
CREATE EXTENSION IF NOT EXISTS pg_net;

-- Função corrigida que só envia email se pagamento foi confirmado via webhook
...
BEGIN
  ...
  IF NEW.status = 'paid' AND (OLD.status IS NULL OR OLD.status != 'paid') THEN
    ...
    -- Só enviar email se o pagamento foi realmente confirmado via webhook
