-- ==========================================
-- CORREÇÃO: Trigger aceita cakto_payment_status = 'approved' mesmo sem cakto_transaction_id
-- ==========================================
-- PROBLEMA: O trigger só envia email se tiver cakto_transaction_id E cakto_payment_status = 'approved'
-- Mas alguns pagamentos podem ter cakto_payment_status = 'approved' sem transaction_id imediatamente
-- 
-- SOLUÇÃO: Aceitar cakto_payment_status = 'approved' mesmo sem cakto_transaction_id
-- Isso alinha com a correção já feita no validate_cakto_order
-- ==========================================

-- Função corrigida que aceita cakto_payment_status = 'approved' mesmo sem transaction_id
...
BEGIN
  ...
  IF NEW.status = 'paid' AND (OLD.status IS NULL OR OLD.status != 'paid') THEN
    ...
    -- Aceitar cakto_payment_status = 'approved' mesmo sem cakto_transaction_id
    v_has_valid_payment_indicator := (
      -- Cakto: aceitar se tiver transaction_id E status approved OU apenas status approved
      (
        ...
        OR
        (NEW.cakto_payment_status IN ('approved', 'paid', 'pago', 'aprovada'))
      )
      OR
    ...
    -- ✅ CORREÇÃO: Se não tem indicador válido, verificar se letra já foi gerada
    -- Se letra já foi gerada, não fazer nada (webhook já processou)
    -- Se letra NÃO foi gerada, tentar gerar mesmo sem indicador válido (fallback)
    IF NOT v_has_valid_payment_indicator THEN
      -- Verificar se letra já foi gerada ANTES de retornar
      SELECT EXISTS (
        SELECT 1 
        FROM jobs j
      ...
      -- Se letra já foi gerada, não fazer nada (webhook já processou)
      IF v_has_lyrics_generated THEN
        RAISE NOTICE '⚠️ [Trigger] Pedido % marcado como paid mas SEM indicador válido. Letra já gerada, pulando. (Cakto: %, Stripe: %)', 
          v_order_id, 
          NEW.cakto_payment_status,
          CASE WHEN NEW.stripe_payment_intent_id IS NOT NULL THEN 'SIM' ELSE 'NÃO' END;
        RETURN NEW;
      END IF;
      
      -- Se letra NÃO foi gerada, tentar gerar mesmo sem indicador válido (fallback)
      RAISE NOTICE '⚠️ [Trigger] Pedido % marcado como paid mas SEM indicador válido. Tentando gerar letra como fallback. (Cakto: %, Stripe: %)', 
        v_order_id, 
        NEW.cakto_payment_status,
        CASE WHEN NEW.stripe_payment_intent_id IS NOT NULL THEN 'SIM' ELSE 'NÃO' END;
      -- Continuar para tentar gerar letra (não retornar aqui)
    END IF;
    ...
    -- ✅ CORREÇÃO: Melhorar verificação para considerar jobs com qualquer status exceto 'failed'
    SELECT EXISTS (
      SELECT 1 
      FROM jobs j
      WHERE j.order_id = v_order_id
    ...
    -- ✅ CORREÇÃO: Só enviar email se tiver indicador válido de pagamento confirmado
...
'Função do trigger que executa o fluxo completo de pagamento quando pedido é marcado como paid: envia email/WhatsApp via notify-payment-webhook (se não enviado E tem indicador válido - aceita cakto_payment_status = approved mesmo sem transaction_id) e SEMPRE inicia geração de música via generate-lyrics-for-approval (se não gerada, mesmo sem indicador válido como fallback). Usa pg_net para chamar Edge Functions.';

COMMENT ON TRIGGER trigger_complete_payment_flow ON orders IS 
'Trigger que dispara fluxo completo de pagamento (email, WhatsApp e geração de música) quando status muda para paid. Email só é enviado se pagamento foi confirmado via webhook (tem indicadores válidos - aceita cakto_payment_status = approved mesmo sem transaction_id). Letra SEMPRE é gerada se não foi gerada ainda (mesmo sem indicador válido como fallback). Verifica se email/letra já foram processados antes de executar.';
