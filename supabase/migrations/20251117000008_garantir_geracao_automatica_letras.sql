-- ==========================================
-- GARANTIR GERAÇÃO AUTOMÁTICA DE LETRAS
-- ==========================================
-- Este script garante que o trigger está funcionando corretamente
-- para gerar letras automaticamente quando pedido é marcado como pago
-- ==========================================

-- ==============================
-- PARTE 1: GARANTIR QUE PG_NET ESTÁ HABILITADO
-- ==============================

CREATE EXTENSION IF NOT EXISTS pg_net WITH SCHEMA extensions;

-- ==============================
-- PARTE 2: RECRIAR TRIGGER COM LÓGICA CORRIGIDA
-- ==============================
...
BEGIN
  ...
  IF NEW.status = 'paid' AND (OLD.status IS NULL OR OLD.status != 'paid') THEN
    ...
    -- ✅ CORREÇÃO: Verificar se letra já foi gerada (verificar jobs E lyrics_approvals)
    SELECT EXISTS (
      SELECT 1 FROM jobs 
      WHERE order_id = v_order_id 
        AND status != 'failed'
    ...
    -- ✅ PASSO 2: SEMPRE gerar letra se não foi gerada (CRÍTICO)
    IF NOT v_has_lyrics_generated THEN
      BEGIN
        RAISE NOTICE '🎵 [Trigger] Iniciando geração de letra para pedido %', v_order_id;
        ...
        BEGIN
          INSERT INTO admin_logs (
            ...
            jsonb_build_object(
              ...
              'has_lyrics_generated', v_has_lyrics_generated,
...
'Função do trigger que executa o fluxo completo de pagamento quando pedido é marcado como paid: envia email via notify-payment-webhook (se tem indicador válido) e SEMPRE gera letra via generate-lyrics-for-approval (se não foi gerada). Usa pg_net para chamar Edge Functions.';

COMMENT ON TRIGGER trigger_complete_payment_flow ON orders IS 
'Trigger que dispara fluxo completo de pagamento (email e geração de música) quando status muda para paid. Email só é enviado se pagamento foi confirmado via webhook (tem indicadores válidos). Letra SEMPRE é gerada se não foi gerada ainda. Verifica se email/letra já foram processados antes de executar.';
