-- ==========================================
-- ATUALIZAR DELIVERY TIME PARA 48 HORAS NO PLANO EXPRESS
-- Atualiza a função get_order_paid_variables para retornar 48 horas
-- ==========================================

-- Atualizar função get_order_paid_variables para retornar 48 horas no express
...
BEGIN
  ...
  -- ✅ ATUALIZADO: Express é 48 horas (conforme solicitado)
  IF order_data.plan = 'express' THEN
    delivery_time_text := CASE COALESCE(user_data.language, 'pt')
      WHEN 'pt' THEN '48 horas'
      WHEN 'es' THEN '48 horas'
      WHEN 'en' THEN '48 hours'
      ELSE '48 hours'
    END;
  ELSE
    delivery_time_text := CASE COALESCE(user_data.language, 'pt')
  ...
  result := jsonb_build_object(
    ...
    'plan', CASE 
      ...
      WHEN order_data.plan = 'express' THEN (
        CASE COALESCE(user_data.language, 'pt')
          WHEN 'pt' THEN 'Express (48 horas)'
          WHEN 'es' THEN 'Expreso (48 horas)'
          WHEN 'en' THEN 'Express (48 hours)'
          ELSE 'Express (48 hours)'
        END)
      ELSE (
        CASE COALESCE(user_data.language, 'pt')
          WHEN 'pt' THEN 'Standard (7 dias)'
...
'Extrai variáveis para template order_paid. Express plan retorna delivery_time: 48 horas (pt), 48 horas (es), 48 hours (en). Standard plan retorna: 7 dias (pt), 7 días (es), 7 days (en)';
